...
let hm=(D.liquidationMap?.[A]||[]).slice().sort((x,y)=>y.estimatedUsd-x.estimatedUsd),maxH=Math.max(1,...hm.map(x=>x.estimatedUsd||0));$("heat").innerHTML=hm.length?hm.slice(0,24).map(x=>{let w=Math.max(4,100*(x.estimatedUsd||0)/maxH);return "<div style='padding:7px 10px;border-bottom:1px solid #12233d'><div style='display:flex;justify-content:space-between;gap:8px'><b class="+(x.side==="LONG"?"g":"r")+">"+x.side+" LIQ</b><span>$"+Math.round(x.price).toLocaleString()+" · "+M(x.estimatedUsd)+"</span></div><div class=bar style='margin-top:4px'><i style='width:"+w+"%'></i></div></div>"}).join(""):"<div class=muted style='padding:18px'>正在累積 OI Δ；至少需要兩次 OI snapshot 才會產生清算資金強度。</div>";
...
server.listen(PORT,"0.0.0.0",()=>console.log("WhaleScope v9.3 nearest liquidation target paper test on",PORT));
