...
fetch("/api/snapshot").then(r=>r.json()).then(j=>{trades=j.trades||[];let h=j.health||{};$("health").textContent=Object.values(h).filter(v=>String(v).toLowerCase()==="online").length+"/"+Object.keys(h).length+" SOURCES";$("status").innerHTML=Object.entries(h).map(([k,v])=>"<div style='padding:8px 0;border-bottom:1px solid #14233d'>"+k+" <span style='float:right' class='green'>"+v+"</span></div>").join("");$("sources").innerHTML=Object.keys(h).map(k=>"<div style='margin:12px 0'>"+k+"<div class='bar'><i style='width:50%'></i></div></div>").join("");draw()});
