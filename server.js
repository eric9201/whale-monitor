...
</style></head><body><main><div class=head><div><div class=logo>◉ WhaleScope</div><div class=muted>Verified CEX + On-chain Intelligence</div></div><select id=a><option>BTC</option><option>ETH</option></select><span class=pill>INTEGRATED WHALE FLOW</span><b class=g>● <span id=live>CHECKING</span></b></div>
...
<div class=grid><div class=card><div class=title>預估清算資金熱圖</div><div id=heat class=heat></div><div class=muted>ESTIMATED LIQUIDATION HEATMAP：Binance / Bybit / OKX 公開 OI Δ 建模；槓桿僅在模型內計算，畫面整合為價格區間與預估清算資金強度。</div></div><div class=card><div class=title>實際公開強平</div><div id=liq class=scroll></div></div></div>
...
let hm=D.liquidationMap?.[A]||[],last=D.trades.filter(x=>x.asset===A).at(-1)?.price||0,ps=hm.map(x=>x.price),lo=ps.length?Math.min(last||Infinity,...ps):0,hi=ps.length?Math.max(last||0,...ps):0,sp=hi-lo||1,maxH=Math.max(1,...hm.map(x=>x.estimatedUsd||0));$("heat").innerHTML=hm.length?hm.slice(0,60).map(x=>{let top=(hi-x.price)/sp*92+4,w=18+82*((x.estimatedUsd||0)/maxH);return "<div class='z "+(x.side==="LONG"?"zl":"zs")+"' style='top:"+top+"%;width:"+w+"%'>"+x.side+" LIQ · $"+Math.round(x.price).toLocaleString()+" · "+M(x.estimatedUsd)+"</div>"}).join(""):"<div class=muted style='padding:18px'>正在累積 OI Δ；至少需要兩次 OI snapshot 才會產生清算資金強度。</div>";
...
server.listen(PORT,"0.0.0.0",()=>console.log("WhaleScope v9 Flow Lab OOS validator on",PORT));
