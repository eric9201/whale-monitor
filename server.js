...
let hm=(D.liquidationMap?.[A]||[]).slice(),last=mid(A);
if(hm.length){
  ...
  $("heat").innerHTML=placed.map(x=>{
    let w=Math.max(10,Math.min(94,94*x.estimatedUsd/maxH)),
        cls=x.side==="LONG"?"hlong":"hshort";
    return "<div class='"+cls+"' style='position:absolute;left:0;top:"+x.y+"px;width:"+w+"%;height:22px;line-height:22px;padding-left:12px;white-space:nowrap;overflow:visible;z-index:2'>"+
...
server.listen(PORT,"0.0.0.0",()=>console.log("WhaleScope v9.4 clean chart heatmap + nearest liquidation target on",PORT));
