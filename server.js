...
app.get("/api/snapshot",(_,r)=>{const c=now()-86400000, recent=trades.filter(x=>x.ts>=c);
 ...
 marketDistribution,liquidationMap:{status:"ESTIMATED_OI_MODEL",BTC:heat("BTC"),ETH:heat("ETH")},zones:{BTC:zones("BTC"),ETH:zones("ETH")}})});
...
<div class=grid><div class=card><div class=title>清算槓桿參考區</div><div id=heat class=heat></div><div class=muted>ESTIMATED LIQUIDATION HEATMAP：Binance / Bybit / OKX 公開 OI Δ 建模；槓桿僅在模型內計算，畫面整合為價格區間與預估清算資金強度。</div></div><div class=card><div class=title>實際公開強平</div><div id=liq class=scroll></div></div></div>
...
async function snap(){try{D=await fetch("/api/snapshot",{cache:"no-store"}).then(r=>r.json());draw()}catch(e){$("live").textContent="API ERROR"}}snap();setInterval(snap,5000);$("a").onchange=draw;
...
server.listen(PORT,"0.0.0.0",()=>console.log("WhaleScope v7 integrated OI heatmap on",PORT));
