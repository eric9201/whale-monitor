...
function aggregateWalls(asset,minNotional=0){
  ...
  return [...groups.values()].map(g=>({...g,price:g.weight?g.priceNum/g.weight:0,durationMs:t-g.firstSeen,venueCount:g.venues.length}))
    .filter(g=>g.current>=minNotional).sort((a,b)=>b.current-a.current);
}
...
// Binance
connect("Binance",()=>{const ws=new WebSocket("wss://fstream.binance.com/stream?streams=btcusdt@aggTrade/ethusdt@aggTrade/btcusdt@forceOrder/ethusdt@forceOrder/btcusdt@depth20@500ms/ethusdt@depth20@500ms");ws.on("message",b=>{try{const j=JSON.parse(b),x=j.data,st=j.stream||"",A=st.startsWith("eth")?"ETH":"BTC";if(st.includes("@aggTrade"))putTrade("Binance",A,x.m?"SELL":"BUY",x.p,x.q,x.a,x.T);else if(st.includes("@forceOrder")){const o=x.o;putLiq("Binance",A,o.S,+o.ap||+o.p,o.q,o.T)}else if(st.includes("@depth20"))putBook("Binance",A,(x.b||[]).map(v=>[+v[0],+v[1]]),(x.a||[]).map(v=>[+v[0],+v[1]]))}catch(e){state.Binance.error=String(e)}});return ws});
...
app.get("/api/snapshot",(_,r)=>{const c=now()-86400000, recent=trades.filter(x=>x.ts>=c);
 ...
 marketDistribution, liquidationMap:{status:"UNAVAILABLE_NO_OI_SOURCE",message:"尚未接入可驗證 OI/清算強度資料，因此不顯示虛構清算金額。"},zones:{BTC:zones("BTC"),ETH:zones("ETH")}})});
...
<div class=grid><div class=card><div class=title>清算槓桿參考區</div><div id=heat class=heat></div><div class=muted>10x/25x/50x/100x 僅為價格槓桿參考。清算「資金強度」必須來自可驗證 OI/清算資料；目前未接入，因此不偽造金額。</div></div><div class=card><div class=title>實際公開強平</div><div id=liq class=scroll></div></div></div>
...
<div class=card style="margin-top:10px"><div class=title>資金流向 · MONEY FLOW</div><div id=flow class=flow></div></div>
</main><script>
let D={trades:[],liquidations:[],books:{BTC:{},ETH:{}},health:{},zones:{BTC:[],ETH:[]},walls:{BTC:[],ETH:[]},bursts:{BTC:{},ETH:{}},marketDistribution:{},liquidationMap:{}};const $=x=>document.getElementById(x),M=n=>"$"+Intl.NumberFormat("en",{notation:"compact",maximumFractionDigits:2}).format(n||0);
...
function draw(){let A=$("a").value,T=+$("t").value,q=C(1440,A),bur=B(A,T);let wb=bur.filter(v=>v.side==="BUY").reduce((z,v)=>z+v.usd,0),ws=bur.filter(v=>v.side==="SELL").reduce((z,v)=>z+v.usd,0);$("total").textContent=M(wb+ws);$("buy").textContent=M(wb);$("sell").textContent=M(ws);$("net").textContent=(wb-ws>=0?"+":"")+M(wb-ws);$("tr").innerHTML=bur.slice(-40).reverse().map(v=>"<tr><td>"+new Date(v.start).toLocaleTimeString()+"</td><td class="+(v.side==="BUY"?"g":"r")+">"+v.side+"</td><td>"+M(v.usd)+" <span class=muted>("+v.count+" fills)</span></td><td>"+v.exchange+"</td></tr>").join("")||"<tr><td colspan=4 class=muted>等待 3 秒聚合後符合門檻的巨鯨成交…</td></tr>";
let w=(D.walls?.[A]||[]).filter(x=>x.current>=T);$("wa").innerHTML=w.slice(0,50).map(x=>"<tr><td class="+(x.side==="BID"?"g":"r")+">"+x.side+"</td><td>"+x.price.toLocaleString(undefined,{maximumFractionDigits:1})+"</td><td>"+M(x.current)+"<div class=muted>"+x.venueCount+" venues · "+dur(x.durationMs)+" · max "+M(x.max)+"</div></td><td>"+x.venues.map(v=>v.exchange).join(", ")+"</td></tr>").join("")||"<tr><td colspan=4 class=muted>目前沒有符合門檻的持續掛單</td></tr>";
let z=D.zones[A]||[],ps=z.map(x=>x.price),last=D.trades.filter(x=>x.asset===A).at(-1)?.price||0,lo=Math.min(last*.88,...ps),hi=Math.max(last*1.12,...ps),sp=hi-lo||1;$("heat").innerHTML=z.map(x=>"<div class='z "+(x.side==="LONG"?"zl":"zs")+"' style='top:"+((hi-x.price)/sp*100)+"%'>"+x.side+" "+x.lev+"x · "+x.price.toFixed(0)+"</div>").join("");
...
async function snap(){try{D=await fetch("/api/snapshot",{cache:"no-store"}).then(r=>r.json());draw()}catch(e){$("live").textContent="API ERROR"}}snap();setInterval(snap,5000);$("a").onchange=draw;$("t").onchange=draw;
...
server.listen(PORT,"0.0.0.0",()=>console.log("WhaleScope v5 verified pipeline on",PORT));
