...
// Binance
connect("Binance",()=>{const url="wss://fstream.binance.com/public/stream?streams=btcusdt@aggTrade/ethusdt@aggTrade/btcusdt@forceOrder/ethusdt@forceOrder/btcusdt@depth20@100ms/ethusdt@depth20@100ms",ws=new WebSocket(url);state.Binance.endpoint=url;state.Binance.messageCount=0;ws.on("open",()=>console.log("[Binance] OPEN",url));ws.on("message",b=>{try{const j=JSON.parse(b),x=j.data,st=j.stream||"",A=st.startsWith("eth")?"ETH":"BTC";state.Binance.messageCount++;state.Binance.lastMessage=now();state.Binance.lastStream=st;if(state.Binance.messageCount===1)console.log("[Binance] FIRST_MESSAGE",st);if(j.code!=null){state.Binance.error="WS "+j.code+": "+j.msg;console.error("[Binance]",state.Binance.error);return}if(st.includes("@aggTrade"))putTrade("Binance",A,x.m?"SELL":"BUY",x.p,x.q,x.a,x.T);else if(st.includes("@forceOrder")){const o=x.o;putLiq("Binance",A,o.S,+o.ap||+o.p,o.q,o.T)}else if(st.includes("@depth20"))putBook("Binance",A,(x.b||[]).map(v=>[+v[0],+v[1]]),(x.a||[]).map(v=>[+v[0],+v[1]]))}catch(e){state.Binance.error=String(e);console.error("[Binance] PARSE",e)}});ws.on("close",(code,reason)=>console.log("[Binance] CLOSE",code,String(reason)));ws.on("error",e=>console.error("[Binance] ERROR",e.message));return ws});
...
server.listen(PORT,"0.0.0.0",()=>console.log("WhaleScope v9.7 Binance validated stream config on",PORT));
